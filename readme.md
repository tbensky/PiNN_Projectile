...
Here, we demonstrate this idea by using a neural network to "learn" the trajectory of a projectile subject to the drag force ${\vec F}_{drag}=C{\vec v^2}$.
...
The results of a numerical integration are [here](https://github.com/tbensky/PiNN_Projectile/blob/main/System/trajectory.csv), for which we plotted the $x$ and $y$ positions of the trajectory:
...
### The loss function
Next, we'll add the loss function. This is different with PiNNs vs 100% data-trained networks, because PiNNs have a net training loss that is combination of both data and physics losses (not just data loss). Thus, you'll have to write a custom loss function. We found with PyTorch, as long as the loss function is a member of the `neural_network` class, the autodifferentiation seems to hold up. 
...
Finally, with a neural network trained on the projectile, we now have a model which can be interrogated by inputting some value of time into the network and watching what comes out of the inputs. 

Here's a couple of plots that ramp the time value (input to the network), while plotting both x and y positions. Another plots $v_x$ and $v_y$.

![Figure 16](https://github.com/tbensky/PiNN_Projectile/blob/main/Media/xy.png)
 ...
 * [Physics-Informed Neural Networks (PINNs) - An Introduction](https://www.youtube.com/watch?v=G_hIppUWcsc&t=863s)
